...
First, we will need to ensure that we depend on:
...
Observability is built-in Spring libraries, providing you with metrics and traces for Spring MVC requests, Spring Batch jobs, Spring Security infrastructure, etc.
...
First, let's add *Micrometer Tracing* and *Zipkin* to our `build.gradle`:
